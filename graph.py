...
def GBFS(gp):
    ...
    while frontier:
        frontier.sort(
            key=lambda node: heuristicSLD[node.state])  # gbfs called heuristic func to sort no other change than ufs
...
def aStar(gp):
    ...
    while frontier:
        frontier.sort(key=lambda node: node.path_cost + heuristicSLD[
            node.state])  # gbfs called heuristic func to sort no other change than ufs
